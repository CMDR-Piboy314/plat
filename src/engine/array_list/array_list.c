...
Array_List* array_list_create(size_t item_size, size_t initial_capacity) {
	...
	if (!list) {
		ERROR_RETURN(NULL, "Failed to allocate memory for Array_List!\n");
	}
...
size_t array_list_append(Array_List* list, void* item) {
	if (list->len == list->capacity) {
		...
		if (!items) {
			ERROR_RETURN(-1, "Failed to allocate memory for Array_List\n");
		}
...
void* array_list_get(Array_List* list, size_t index) {
	if (index >= list->len) {
		ERROR_RETURN(NULL, "Array_List Index out of bounds\n");
	}
...
u8 array_list_remove(Array_List* list, size_t index) {
	if (list->len == 0) {
		ERROR_RETURN(1, "Array_List is empty\n");
	}

	if (index >= list->len) {
		ERROR_RETURN(1, "Array_List Index out of bounds\n");
	}
