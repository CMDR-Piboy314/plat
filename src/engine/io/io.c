...
#define IO_READ_ERROR_MEMORY "Not enough memory free to read file: %s\n"

/* Not using fseek to avoid binary file issues */
	...
	while(true) {
		if (used + IO_READ_CHUNK_SIZE + 1 > size) {
			...
			if (size <= used) {
				...
				ERROR_RETURN(file, "File %s is too large'n", path);
			}
...
int io_file_write(void* buffer, size_t size, const char* path) {
	...
	if (!fp || ferror(fp)) {
		ERROR_RETURN(1, "Can't write to file %s\n", path);
	}
	...
	if (chunks_written != 1) {
		ERROR_RETURN(1, "Write error, expected one chunk, got %zu\n", chunks_written);
	}
